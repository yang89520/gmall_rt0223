...
import com.yiran.gmall.realtime.bean.{OrderDetail, SkuInfo}
import com.yiran.gmall.realtime.util.{MyKafkaSink, MyKafkaUtil, OffsetManagerUtil, PhoenixUtil}
...
object OrderDetailApp {
  def main(args: Array[String]): Unit = {
    ...
    val topic = Constansts.ODS_ORDER_DETAIL
    val groupId = Constansts.ORDER_DETAIL_GROUP
    ...
    orderDetailWithSkuDStream.foreachRDD{
      rdd=>{
        rdd.foreach{
          orderDetail=>{
            MyKafkaSink.send(Constansts.DWD_ORDER_DETAIL,
