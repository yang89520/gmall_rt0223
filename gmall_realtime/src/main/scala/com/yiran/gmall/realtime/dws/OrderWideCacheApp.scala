...
import com.yiran.gmall.realtime.util.{MyKafkaUtil, MyRedisUtil, OffsetManagerUtil}
...
object OrderWideCacheApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /*方案二：
        主表.fullOuterJoin(从表)
        1 join
        2.1 主表写缓存   2.2 主表查缓存   3.1 从表写缓存  3.2 从表读缓存
    */
    ...
    /*方式一：flatmap*/
    /*
    //flatMap不太合理 内部的redis的链接会多次获取 使用mapPartition+foreach比较合适
    val orderWideDstream: DStream[OrderWide] = fullJoinedDstream.flatMap {
      case (orderId, (orderInfoOpt, orderDetailOpt)) =>
        ...
        if (orderInfoOpt != None) {
          ...
          //2.1 主表写缓存#######################
          // redis  type?  string     key ?  order_info:0101    value? orderInfoJson  expire?:600 15分钟
    ...
    */
    /*方式二：优化以分区读取*/
    val orderWideDstream: DStream[OrderWide] = fullJoinedDstream.mapPartitions {
      partition => {
        //分区只创建一个buffer和redis连接
        //buffer保存批次数据
        val orderWideList: ListBuffer[OrderWide] = ListBuffer[OrderWide]()
        val jedis: Jedis = MyRedisUtil.getJedisClient
        partition.foreach {
